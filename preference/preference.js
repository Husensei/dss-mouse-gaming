  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const reset = await client.query(
        `DELETE FROM pairwise_comparison 
...
const insertPreference = async (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    WHERE EXISTS (SELECT 1 FROM preference);`
    );

    client.query(
      `INSERT INTO preference 
      (ergo_pref, ambi_pref, wireless_pref, 
      wired_pref, palm_pref, claw_pref, 
      fingertip_pref, price_pref) VALUES 
      ('${data.ergo_pref}', '${data.ambi_pref}', 
      '${data.wireless_pref}', '${data.wired_pref}', 
      '${data.palm_pref}', '${data.claw_pref}', 
      '${data.fingertip_pref}', '${data.price_pref}')`
    );
    resolve({ status: 201, message: "Insert success" });
  });
};
