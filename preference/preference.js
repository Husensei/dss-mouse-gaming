...
const insertMatrix = async (data) => {
  return new Promise(async (resolve, reject) => {
    ...
    for (var i = 0; i < data.value.length; i++) {
      queries.push(`INSERT INTO pairwise_comparison (id_criteria1, id_criteria2, value) VALUES ('${data.value[i].row}', '${data.value[i].column}', '${data.value[i].value}'`);
    }
    try {
      ...
      resolve({ staus: 201, message: "Insert success" });
    } catch (error) {
      reject(error);
    }
  });
